...
const qRandom = db.prepare(
  'SELECT id, text FROM splashes WHERE active = 1 ORDER BY RANDOM() LIMIT 1'
);
const qActive = db.prepare('SELECT id, text FROM splashes WHERE active = 1 ORDER BY id ASC');

export function randomSplash() {
  return qRandom.get() || null;
}
